...
if [ -z "$VANILLA_VERSION" ] || [ "$VANILLA_VERSION" -eq "latest" ]; then
...
if [ -f $CHK_FILE ]; then
   ...
   curl https://s3.amazonaws.com/Minecraft.Download/versions/$DL_VERSION/minecraft_server.$DL_VERSION.jar -o server.jar
fi
